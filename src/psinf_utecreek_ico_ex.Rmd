...
cut_clump_fn <- function(
  ...
  lvl_exists <- levels(need_cut_trees$clump_n_trees_lb) %>% 
    as.character() %>% 
    as.numeric() %>% 
  ...
  these_cut_breaks <-
    c(
      0 # always pad with 0
      , levels(need_cut_trees$clump_n_trees_ub) %>% 
        as.character() %>% 
        as.numeric() %>%
        unique() %>% 
        purrr::discard(~ is.null(.x) || is.na(.x))
    ...
    unique() %>% 
    sort() 
  if(identical(these_cut_breaks,c(0,Inf))){
    stop("could not identify cut breaks from `need_cut_trees` data based on 'clump_n_trees_ub' factor levels")
  }
...
#     , tgt = as.numeric(as.character(levels(treetops_clustered$clump_n_trees_lb)))[2]
#   )
...
#     , clump_dist_m = treetops_clustered$clump_dist_m[1]
