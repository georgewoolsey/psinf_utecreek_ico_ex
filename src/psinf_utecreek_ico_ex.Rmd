  ...
  point_clusters <- point_clusters %>% 
    dplyr::bind_cols(
      ...
      dplyr::mutate(
        ...
        clump_n_trees_lb = factor(as.numeric(clump_n_trees_lb)+1, levels = as.character( clump_breaks[-length(clump_breaks)]+1 ) )
        , clump_n_trees_ub = factor(clump_n_trees_ub, levels = as.character(clump_breaks[-1]))
      ) %>% 
...
treetops_clustered <- 
  get_tree_clumps(
    trees_sf = treetops_sf_with_dbh %>% dplyr::slice_sample(n=5555)
    , stand_sf = stand_sf
...
get_target_check_prescription = function(
  ...
  #############################################
    # how many clump groups
    n_clump_n_trees_grp <- levels(clump_n_trees_grp_summary_dta$clump_n_trees_grp) %>% length()
    
    target_data <-  
      # create data for joining if missing clump groups
      dplyr::tibble(
        stand_area_ac = rep(clump_n_trees_grp_summary_dta$stand_area_ac[1],n_clump_n_trees_grp)
        , clump_n_trees_grp = factor(
          c(1:n_clump_n_trees_grp)
