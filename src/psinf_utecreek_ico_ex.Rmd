...
st_clump_points <- function(
    ...
    dplyr::bind_cols(
      ...
      dplyr::mutate(
        ...
        clump_n_trees_lb = factor(
          ...
          , levels = as.character( clump_breaks[-length(clump_breaks)]+1 ) 
          , ordered = T
        )
        , clump_n_trees_ub = factor(
          clump_n_trees_ub
          , levels = as.character(clump_breaks[-1])
          , ordered = T
        )
      ) %>% 
  ...
  # ensure final clump factor is ordered
  xxx
  # point_clusters <- 
  #   point_clusters %>% 
  #   dplyr::mutate(
...
########################################################################################
# desired BA
  target_ba_ft2_per_ac <- 25 # cannot be > current BA
...
dplyr::tibble(
        ...
        clump_n_trees_grp = levels(clump_n_trees_grp_summary$clump_n_trees_grp)
        , clump_n_trees_lb = levels(clump_n_trees_grp_summary$clump_n_trees_lb)
        , clump_n_trees_ub = levels(clump_n_trees_grp_summary$clump_n_trees_ub)
      ) %>% 
      # ensure they are still factors
      dplyr::mutate(
        dplyr::across(
          dplyr::everything()
          , ~factor(.x, levels = .x, ordered = T)
        )
        # get numeric values
...
cut_clump_fn <- function(
  ...
  dist_temp <- curr_dta$clump_dist_m[1]
...
get_keep_tree_flag <- function(
  ...
  , tgt # tgt = clump_n_trees_grp level defined in st_clump_points: ...
    ## ... clump_labels = c("Individual","2-4 trees","5-9 trees","10-15 trees","16-25 trees",">25 trees")
  , clump_dist_m # size (radius) of the epsilon neighborhood = maximum distance between points to add to clump
) {
...
sm_grp_temp = prescription_target_data %>% 
...
grp_temp = prescription_target_data %>% 
...
if( 
  ...
  ( clump_polys %>% dplyr::filter(clump_n_trees_grp == grp_temp) %>% nrow() )
){
...
ttops_temp2 = prescription_trees %>% 
  ...
  st_clump_points(clump_dist_m = clump_dist_m)
clump_polys2 = get_clump_summary(ttops_temp2)
get_clump_dist_rast_ans2 = get_clump_dist_rast(clump_polys2)
...
plt_fnl_12 = 
  ggplot() + 
    # distance
    geom_tile(
      data = get_clump_dist_rast_ans2$dist_rast %>% terra::aggregate(2, cores = 4) %>% as.data.frame(xy = T) %>% rename(f=3)
    ...
    # openings
    geom_sf(data = get_clump_dist_rast_ans2$openings_vect, mapping = aes(color = openining_area_m2), fill = NA) +
...
plt_open_12 = 
  ggplot() + 
      ...
      # openings
      ggnewscale::new_scale_fill() +
      geom_sf(data = get_clump_dist_rast_ans2$openings_vect, mapping = aes(fill = openining_area_m2), color = NA) +
      scale_fill_gradient(
        low = "gray77", high = "gray11"
        , labels = scales::comma_format(accuracy = 1)
        , name = latex2exp::TeX("opening\narea ($\\m^2$)")
...
```{r}
clump_n_trees_grp_summary2 = get_clump_n_trees_grp_summary(
  trees = ttops_temp2, clumps = clump_polys2
)
